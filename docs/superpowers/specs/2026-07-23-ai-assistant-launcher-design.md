...
- **No `setlocal enabledelayedexpansion`, no `!VAR!` anywhere** — this script has no need to read back a variable it just set within the same block (Codex reads `ACCOUNTING_MCP_TOKEN` from its own process environment at its own startup, not from this script re-reading it), so delayed expansion is deliberately never enabled. That means `!` is simply an inert, literal character here and needs no escaping — a concrete simplification of the escaping contract, not an oversight.
...
- **`call` before the binary path, always** — many Windows CLI tools (npm-installed tools especially) ship as a generated `.cmd`/`.bat` shim rather than a native `.exe`, and invoking a batch file without `call` from inside another batch file does not return control to the caller afterward (irrelevant for the very last line of *this* wrapper today, but `call` is correct and safe for both `.exe` and `.cmd`/`.bat` targets uniformly, so there's no need to detect which kind the resolved binary path is).
- **Non-Codex wrappers carry no secret at all** — they're just `cd /d` + `call`, no `set` line, so referring to them as part of the "secret set" (as an earlier draft did) was inaccurate; only Codex's wrapper is ever a token-bearing file (see Workspace layout and Secret lifecycle above, now corrected to say this).
...
- **Wrapper-content tests, not just adapter-command tests**: render the actual wrapper script text for both `.sh` and Codex/non-Codex `.cmd` variants, using adversarial workspace/binary paths and tokens (e.g. containing `` $(rm -rf ~) ``, `` `whoami` ``, `; echo pwned`, `%PATH%`, a trailing `%`) and assert the rendered file content is byte-for-byte the correctly-escaped literal template — including that `@echo off` is the first line, `cd /d` (not plain `cd`) is used, `call` precedes the binary path, and the non-Codex variant has no `set` line at all. The adapter-command tests only check the `List<String>` passed to `ProcessBuilder`, which is a separate concern from whether the wrapper's own textual content is safe.
...
- `WindowsTerminalAdapter`'s exact constructed command list (`["wt.exe", "-d", workspace, "cmd.exe", "/c", script]`) is asserted directly, so it doesn't silently drift to relying on an implicit/default-profile command-file interpretation.
