...
- `SieExchangeDialog.exportRequested()` calls `previewSieExport(...)` first, and shows exactly one of two non-blocking confirmations before ever calling `exportFiscalYear(...)`:
  ...
  **Threading:** `previewSieExport` scans `voucher_line` for the whole fiscal year inside a DB transaction — this must not run on the EDT. Checked precedent: `previewSieImport(...)` is *always* invoked inside a `SwingWorker` (`runSmartImport`/`runReplacePreview` — `doInBackground()` calls the service, `done()` runs back on the EDT), and `exportFiscalYear(...)` itself is already wrapped in one (`exportRequested()`'s existing `SwingWorker<SieExportResult, Void>`). `previewSieExport` needs the identical treatment, sequenced *before* that existing export worker rather than replacing it:
  1. A `SwingWorker<SieExportPreview, Void>` calls `previewSieExport(fiscalYearId)` in `doInBackground()`.
  2. Its `done()` (on the EDT) inspects the result and shows the appropriate confirmation dialog, if any.
  3. Only once the user confirms (or there was nothing to confirm) does the existing export `SwingWorker<SieExportResult, Void>` get created and `.execute()`d, i.e. the two workers run one after the other, not concurrently.
