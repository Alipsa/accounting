...
- `SieExchangeDialog.exportRequested()` calls `previewSieExport(...)` first, and shows exactly one of two non-blocking confirmations before ever calling `exportFiscalYear(...)`:
  ...
  **Threading:** `previewSieExport` scans `voucher_line` for the whole fiscal year inside a DB transaction — this must not run on the EDT. Checked precedent: `previewSieImport(...)` is *always* invoked inside a `SwingWorker` (`runSmartImport`/`runReplacePreview` — `doInBackground()` calls the service, `done()` runs back on the EDT), and `exportFiscalYear(...)` itself is already wrapped in one (`exportRequested()`'s existing `SwingWorker<SieExportResult, Void>`, with `setWorkingState(true)` called right before `.execute()` at line 558 and `setWorkingState(false)` called unconditionally as the *first* line of `done()` at line 568, before even checking success/failure). `previewSieExport` needs the identical treatment, sequenced *before* that existing export worker rather than replacing it:
  1. `setWorkingState(true)` is called once, right after the file chooser is confirmed (where `exportRequested()` calls it today) — before the preview worker starts, disabling the export controls for the whole preview→confirm→export sequence, not just the export half of it.
  2. A `SwingWorker<SieExportPreview, Void>` calls `previewSieExport(fiscalYearId)` in `doInBackground()`.
  3. Its `done()` (on the EDT) **must restore `setWorkingState(false)` on every path that does *not* lead to the export worker starting** — the preview failing (`InterruptedException`/`ExecutionException`, same catch shape as the existing export worker's `done()`) and the user cancelling either confirmation dialog. Without this, the dialog is left permanently disabled after a cancel or a preview error, since nothing else would ever call `setWorkingState(false)`.
  4. Only when there's nothing to confirm, or the user confirms, does the existing export `SwingWorker<SieExportResult, Void>` get created and `.execute()`d — and since working-state is already `true` from step 1, this path does **not** call `setWorkingState(true)` again; the export worker's own `done()` already unconditionally calls `setWorkingState(false)` when it finishes, so the two workers run strictly one after the other under a single continuous "working" span rather than toggling off and back on between them.
...
- **New account (insert branch only):** read `account.getSRU()` from the imported `SieAccount`. Each code is passed through `AccountService.normalizeSruCode(...)` then checked with `isValidSruCode(...)` — the same shared helpers `updateAccount` uses (§2), so the format rule can't drift between the manual and import paths. A code that fails validation is **dropped, not persisted**, and adds a warning to the existing `warnings` list (e.g. "Konto ${accountNumber}: ogiltig SRU-kod '${code}' i importfilen hoppades över.") — it does not abort the import, matching how other per-row issues in this method are already handled. The first/second-code assignment rule (first → `sru_code`, second → `sru_code2`, more than two → persist the first two and warn about the rest) is applied to the **surviving valid codes only**, after invalid ones have been filtered out. Empty/null list → both stay `null`.
  ...
  **Plumbing note:** `upsertAccounts(sql, fiscalYearId, accounts)` doesn't currently take a `warnings` parameter, and its one call site — `importDocument(sql, fiscalYearId, document, warnings)` — doesn't pass one, even though `importDocument` already threads `warnings` through to `persistOpeningBalances(sql, fiscalYearId, document.getIB(), warnings)` on the very next line. This spec requires changing the signature to `upsertAccounts(sql, fiscalYearId, accounts, warnings)` and updating that call site to pass it — without this, the warning behavior described above has nowhere to write to.
...
- `SieExchangeDialogTest.groovy`: update if it snapshots dialog fields affected by these changes; add coverage that `exportRequested()` calls `previewSieExport()` first (via its own `SwingWorker`, not synchronously on the EDT) and shows the legal-form-unset confirmation, the missing-codes confirmation, or neither, only starting the existing export worker when there's nothing to confirm or the user confirms; and that `setWorkingState(false)` is restored (dialog re-enabled) on a preview failure and on cancelling either confirmation dialog, not just on export completion.
...
- `upsertAccounts(...)` needs a `warnings` parameter (and its call site in `importDocument` needs to pass one) — otherwise §6's "invalid imported SRU code adds a warning" behavior has no list to write to.
- `previewSieExport(...)` runs inside its own `SwingWorker`, sequenced before the existing export `SwingWorker` (not synchronously on the EDT, and not concurrently with the export worker) — matching how `previewSieImport(...)` is already always invoked on the import side.
- `setWorkingState(true)` is called once, before the preview worker starts; `setWorkingState(false)` must be restored by the preview worker's `done()` on every path that doesn't lead into the export worker (preview error, or the user cancelling either confirmation) — otherwise a cancelled/failed preview leaves the dialog permanently disabled, since nothing else would call `setWorkingState(false)` in that case. The confirm-and-proceed path leaves working-state untouched (already `true`) and relies on the export worker's own unconditional `setWorkingState(false)` at the top of its `done()`.
