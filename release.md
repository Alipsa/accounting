...
- Local install script — Added `localInstall.sh` for developers who want to build and install/update the app locally without creating a formal release. It builds the current platform package, extracts it to `~/.local/lib/AlipsaAccounting`, and registers a desktop entry on Linux.
