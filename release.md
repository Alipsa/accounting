...
- Local update script — Added `localUpdate.sh` that updates an existing installation by auto-discovering it from desktop/Start Menu shortcuts or common install locations on Linux, Windows (Git Bash), and macOS. Supports `--dir <path>` to skip discovery.
