...
| `list_companies` | All companies registered in this installation, including legal form; empty list if none exist yet |
| `get_company_info` | Company name, currency, VAT periodicity, and legal form (AB/Handelsbolag/Enskild firma) |
...
| `get_general_ledger` | Full posting history with running balance; optional account_number filter |
...
| Tool | Purpose |
|------|---------|
| `set_active_voucher_draft` | Fill the unsaved GUI voucher; optional attachment_path auto-attaches a receipt when the user saves; the user must review and save it |
...
2. Load company conventions.
   ...
   If you look at another company's vouchers or general ledger (via `list_companies`, `list_vouchers`, `get_general_ledger`) as a reference for how something was booked, first check that company's `legal_form` via `get_company_info` or `list_companies`. Equity, tax, and owner-transaction account usage differs between `AKTIEBOLAG`, `HANDELSBOLAG_KB`, and `ENSKILD_FIRMA` - never copy one legal form's pattern onto a company with a different legal form.
...
## Workflow: Receipt/invoice image to voucher

1. Read the image the user pasted, or the file path they gave you, to identify date, counterparty, amount, and likely accounts.
2. Follow Workflow: Voucher Entry steps 1-4 (context, conventions, propose, `preview_voucher`).
3. Call `set_active_voucher_draft` including `attachment_path` set to the absolute path of the receipt/invoice file. The path is validated immediately; if it does not point to a readable file, the tool returns `ok: false` - ask the user for the correct path rather than guessing one.
4. Tell the user the draft is ready for review in the desktop application, and that saving it will automatically attach the receipt - there is no separate attach step or tool.
