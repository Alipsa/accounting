...
final class SieImportExportService {
  ...
  private final SieImportJobRepository importJobRepository
  ...
  SieImportExportService(
    ...
    this.importJobRepository = new SieImportJobRepository(databaseService)
  }
  ...
  SieImportPreview previewSieImport(long companyId, Path filePath, boolean replaceExisting) {
    ...
    databaseService.withSql { Sql sql ->
      ...
      ImportJob duplicate = SieImportJobRepository.findSuccessfulByChecksum(sql, companyId, checksum)
  ...
  private SieImportResult importFile(long companyId, Path filePath, boolean replaceExistingFiscalYear, boolean reopenFirst) {
    ...
    try {
      ...
      SieImportResult result = databaseService.withTransaction { Sql sql ->
        ...
        ImportJob job = SieImportJobRepository.complete(
            sql,
            jobId,
            fiscalYear.id,
      ...
      importJobRepository.markFailed(jobId, resolvedFiscalYearId, exception)
      throw exception
    }
  }
